...
const holdToEarnCampaigns = [
  { name: 'Deposit WLD on Morpho', apy: '4.25', protocol: 'Morpho', status: 'Earning', claimCountdown: '2h 45m', claimProgress: 0.31, image: '/morpho.svg' },
  { name: 'Supply USDC on Aave', apy: '5.60', protocol: 'Aave', status: 'Earning', claimCountdown: '1h 12m', claimProgress: 0.72, image: '/aave.png' },
  { name: 'Stake ETH on Lido', apy: '3.10', protocol: 'Lido', status: 'Earning', claimCountdown: '3h 58m', claimProgress: 0.01, image: '/lido.jpg' },
];

const singleClaimRewards = [
  { name: 'Borrow on Morpho', reward: 'Get 5% back', stat: '$50 claimed', image: '/morpho.svg' },
  { name: 'Borrow on Credit', reward: 'Get 5% back', stat: '$7 claimed', image: '/credit.png' },
  { name: 'Swap on Odos', reward: 'Earn $0.25', stat: '$890 claimed', image: '/odos.png', bgBlack: true },
];

/* ───── Progress Ring ───── */
function MiniProgressRing({ progress, size = 36 }: { progress: number; size?: number }) {
  const strokeWidth = 3;
...
function ProjectIcon({ src, bgBlack }: { src: string; bgBlack?: boolean }) {
  return (
    <div className={`w-12 h-12 rounded-[10px] overflow-hidden shrink-0 flex items-center justify-center ${bgBlack ? 'bg-black' : ''}`}>
      <img src={src} alt="" className={bgBlack ? 'w-7 h-7 object-contain' : 'w-full h-full object-cover'} />
    </div>
  );
}
    ...
    <div className="phone-frame">
      <div className="phone-screen">
        ...
        <div className="flex items-center justify-between px-5 pt-4 pb-2">
          <Link
            href="/"
            className="text-[13px] text-[#9CA2AD] font-medium"
          >
            &larr; All mockups
          </Link>
        ...
        <div className="px-5 pt-4 pb-2">
          <p className="text-[14px] text-[#72767f] font-medium">Lifetime earnings</p>
          <p className="text-[32px] font-semibold text-black tracking-tight">$20.93</p>
        </div>
        ...
        <div className="px-5 pt-6 pb-2">
          ...
          <div className="flex gap-1 items-center mt-2">
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
              <circle cx="9" cy="9" r="7.5" stroke="black" strokeWidth="1.2" />
              <ellipse cx="9" cy="9" rx="3" ry="7.5" stroke="black" strokeWidth="1.2" />
          ...
          <div className="flex flex-col gap-[17px]">
            {singleClaimRewards.map((reward, i) => (
              <div key={i} className="flex gap-3 items-center w-full">
                ...
                <ChevronRight className="w-[18px] h-[18px] text-black shrink-0" strokeWidth={2.5} />
              </div>
            ))}
          </div>
        </div>
