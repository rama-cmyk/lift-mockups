...
import { ArrowLeft, MoreHorizontal, ArrowDownToLine, Timer, Gift } from 'lucide-react';
...
function EarnCycleStepper() {
  const steps = [
    { icon: ArrowDownToLine, label: 'Deposit', desc: 'Add funds', status: 'completed' as const },
    { icon: Timer, label: 'Hold', desc: 'Earn rewards', status: 'current' as const },
    { icon: Gift, label: 'Claim', desc: 'Every 4 hours', status: 'upcoming' as const },
  ];
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative flex items-start justify-between px-6">
        ...
        {/* Step columns */}
        {steps.map((step, i) => (
          <div key={i} className="flex flex-col items-center relative z-10" style={{ width: 80 }}>
            <div
            ...
            >
              {step.status === 'current' ? (
                ...
                >
                  <step.icon size={24} className="text-[#56B548]" strokeWidth={2} />
                </motion.div>
              ) : (
                <step.icon
                  size={24}
                  className={step.status === 'completed' ? 'text-white' : 'text-[#9CA2AD]'}
                  strokeWidth={2}
                />
              )}
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 pt-5 pb-32">
          ...
          <div className="flex flex-col items-center text-center mb-6">
            <div className="w-16 h-16 rounded-[10px] overflow-hidden mb-3">
              <img src="/morpho.svg" alt="Morpho" className="w-full h-full object-cover" />
            </div>
          ...
          <div className="rounded-xl bg-[#56B548]/10 border border-[#56B548]/20 px-4 py-3 flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              <motion.div
                ...
                transition={{ duration: 2, repeat: Infinity }}
              />
              <span className="font-semibold text-[15px] text-[#3d6e36]">Earning</span>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-[15px] text-black">{campaign.protocol} Base APY</span>
                </div>
                <span className="font-semibold text-[15px] text-black">{campaign.apy.base}%</span>
              </div>
                ...
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full bg-[#56B548]/15 flex items-center justify-center">
                    <svg width="12" height="12" viewBox="0 0 10 10" fill="none">
                      ...
                      <path d="M1 6.5L5 9L9 6.5" stroke="#56B548" strokeWidth="1" />
                    </svg>
                  </div>
                  <span className="font-medium text-[15px] text-black">Boost Reward</span>
                </div>
                <span className="font-semibold text-[15px] text-[#56B548]">+{campaign.apy.boost}%</span>
              </div>
              <div className="h-px bg-[#F3F4F4]" />
              {/* Total */}
              <div className="flex items-center justify-between">
                <span className="font-semibold text-[15px] text-black">Total APY</span>
                <span className="font-bold text-[17px] text-[#56B548]">{campaign.apy.total}%</span>
              </div>
            </div>
          </div>
          ...
          <div className="my-5">
            ...
            <p className="text-[13px] text-[#72767f] font-medium mt-4 text-center leading-relaxed">
              Rewards unlock every 4 hours.<br />
          ...
          <div className="flex flex-col gap-3 my-4">
            ...
            <DetailRow label="Remaining" value={`${campaign.remaining} ${campaign.rewardToken}`} />
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span
                ...
                style={{ clipPath: `inset(0 ${100 - claimProgressPercent}% 0 0)` }}
              >
                Claim in {campaign.claimCountdownShort}
              </span>
            </button>
